...
//Drop-down lists
$regionQuery = returnDropDown(returnArray("select DISTINCT region_name
	from region order by region_name;", $dbconn));
$t->setVariable ("regionSelect", $regionQuery);

$grapeQuery = returnDropDown(returnArray("select DISTINCT variety
        from grape_variety order by variety;", $dbconn));
$t->setVariable ("grapeSelect", $grapeQuery);

$yearQuery = returnDropDown(returnArray("select DISTINCT year
